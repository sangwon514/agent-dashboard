...
이 스킬의 모든 sub-agent dispatching 은 **parent Codex (이 SKILL.md 를 읽는 컨텍스트) 가 직접 수행**한다.
Codex 는 sub-agent 에게 Agent tool 을 grant 하지 않으므로, sub-agent 가 다른 sub-agent 를 부를 수 없다.
...
PROJECT_DIR="$HOME/.Codex/projects/${PROJECT_ENCODED}"
...
1. **Output-to-File-First**: "50줄+ 출력은 `/tmp/agentville-out/<name>.txt` 로 먼저 쓰고 Grep/Read." (AGENTS.md 인용)
...
- 위험 명령은 부모 Codex 의 기본 가드 + `pre-commit-check.sh` 훅에 위임.
