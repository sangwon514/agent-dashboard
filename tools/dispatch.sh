...
case "$TOOL" in
  ...
  cursor) ( cd "$ROOT" && script -q /dev/null cursor-agent -p -f --model "${CURSOR_MODEL:-claude-opus-4-8-thinking-high}" --output-format text "$PROMPT" </dev/null ) 2>&1 | tee "$LOG"; rc=${PIPESTATUS[0]};;
esac
set -e
