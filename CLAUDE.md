# CLAUDE.md — agent-dashboard (Agentville)

## 🎯 프로젝트 컨셉 (한 줄)

**Agentville** — 모든 Claude Code 세션의 서브에이전트 활동을 펫-키퍼 메타포로 라이브 표시하는 1인 로컬 모니터.
...
이 저장소는 멀티 툴 환경이다(**Claude Code = 머리, Codex/Cursor = 손**). 나(Claude Code)는 계획·아키텍처 결정·SSOT 관리·오케스트레이션(`/auto`,`/propose`)·최종 리뷰를 맡는다. 명확히 정의된 구현은 손(Cursor/Codex)에게 위임 가능하며, 손의 결과물은 `git diff` 로 리뷰한다. 역할 분담 SSOT 는 [`COLLAB.md`](./COLLAB.md).
...
| `pre-compact-save.sh` | PreCompact | git 상태 → `.claude/compact-state.md` 자동 저장 |
...
## 🔗 서브에이전트 간 정보 공유 규칙 (`.claude/scratch/`)
...
`/auto` 는 자율 루프 슬래시 스킬 (`.claude/skills/auto/SKILL.md`). **모든 fan-out 은 parent Claude 가 수행** (sub-agent 가 Agent tool 을 못 받기 때문).
...
- `git push --force` / `rm -rf` / DB-style 위험 명령은 부모 Claude 의 기본 가드만 사용 (별도 hook 미설정 — 1인 로컬이라 과한 보호 불필요)
...
1. 해당 디렉토리에 파일 추가
2. 본 CLAUDE.md 카탈로그 표 갱신
3. `.claude/WORKFLOW.md` 도 갱신
4. 메모리(`~/.claude/projects/-Users-sangwonlee-agent-dashboard/memory/`) 의 관련 항목 갱신 — 컨셉이 바뀌었다면 `project_agentville.md` 도 함께
